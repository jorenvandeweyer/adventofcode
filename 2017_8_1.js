...
for(let i = 0; i < x.length; i++){
    ...
    if(memory[instruction.condition.memory] == undefined){
        memory[instruction.condition.memory] = 0;
    }


    if(memory[instruction.memory] == undefined){
        memory[instruction.memory] = 0;
    }

    if(eval(memory[instruction.condition.memory] + instruction.condition.comparisation + instruction.condition.value)){
        switch (instruction.instruction) {
            case "inc":
                memory[instruction.memory] += instruction.value;
                if(memory[instruction.memory] > highestValue){
                    highestValue = memory[instruction.memory];
                }
                break;
            case "dec":
            memory[instruction.memory] -= instruction.value;
...
console.log(biggestValue(memory));
console.log(highestValue)

function biggestValue(mem){
    let arr = Object.keys( mem ).map(function ( key ) { return mem[key]; });
...
function parseString(str){
    let reg = str.split(" if ");

    let instruction = reg[0].split(" ");
    let condition = reg[1].split(" ");
    ...
    return {
        ...
        condition: {
            memory: condition[0],
            comparisation: condition[1],
            value: parseInt(condition[2])
        }
    };
}
