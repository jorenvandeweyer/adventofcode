let x = `suvtxzq (242) -> tdoxrnb, oanxgk
smjsfux (7)
oanxgk (68)
tvvfszm (66)
...
let bigTower = appendTower(x);

addSumWeight(bigTower);
console.log(bigTower);

function addSumWeight(obj){
    for(let key in obj){
        if(!obj[key].childrenKey){
            obj[key].sumWeight = obj[key].weight;
        } else {
            if(!haveAllSumWeight(obj[key].children)){
                addSumWeight(obj[key].children);
            }
            obj[key].sumWeight = obj[key].weight + haveAllSumWeight(obj[key].children);
        }
    }
}

function haveAllSumWeight(obj){
    let weight = 0;
    let prev = 0;
    for(let key in obj){
        if(obj[key].sumWeight == undefined){
            return false;
        }
        if(prev != obj[key].sumWeight && prev != 0){
            for(mistake in obj){
                console.log("key:", mistake, "Sweight:", obj[mistake].sumWeight, "weight:", obj[mistake].weight);
            }
            console.log("---");
        }
        prev = obj[key].sumWeight;
        weight += obj[key].sumWeight
    }
    return weight;
}

function appendTower(arr, obj={}){
    ...
    for(let i = 0; i<arr.length; i++){
        ...
        for(let key in obj){
            if(tower.children){
                if(tower.children.includes(key)){
                    arr.splice(i, 1);
                    let newObj = {};
                    newObj[tower.key] = {
                        key: tower.key,
                    ...
                    return appendTower(arr, newObj);
                }
            }
            if(canAppend(tower, obj)){
                arr.splice(i, 1);
                return appendTower(arr, obj);
            }
        }
    }
...
function canAppend(tower, obj){
    for(let key in obj){
        if(obj[key].childrenKey){
            if(obj[key].childrenKey.includes(tower.key)){
                obj[key].children[tower.key] = {
                    key: tower.key,
                    weight: tower.weight,
                    childrenKey: tower.children,
                    children: {}
            ...
            if(canAppend(tower, obj[key].children)){
                return true;
            }
        }
    }
