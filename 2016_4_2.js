let x = `aczupnetwp-mfyyj-opalcexpye-977[peyac]
...
for(let i = 0; i < x.length; i++){
    ...
    let room_checksum = str.splice(str.length -1, 1);
    room_checksum = room_checksum[0].split("[");
    room_checksum[1] = room_checksum[1].slice(0, -1);

    if(checkRoomEncoding(str, room_checksum[1])){
        sum += parseInt(room_checksum[0]);

        decryptRoomName(str, room_checksum[0]);
...
function decryptRoomName(str, sectorId){
    let newStr = []
    str = str.join(" ");
    ...
    for(let i = 0; i < str.length; i++){
        ...
        newStr[i] = String.fromCharCode(char);
    }
    // console.log(newStr.join(""));

    if (newStr.join("").includes("north")){
        console.log(newStr.join(""), sectorId);
    }
}

function checkRoomEncoding(str, encoding){
    str = str.join("");
    let obj = {};
    for(let i = 0; i < str.length; i++){
