const input = `[1518-06-12 23:57] Guard #2633 begins shift
...
const values = input
    ...
    .split("\n")
    .map(line => {
        const dateString = line.split("] ")[0].replace("[", "");
...
const sorted_values = values.sort((e1, e2) => e1.date - e2.date); 
...
for (let i = 0; i < sorted_values.length; i++) {
    const event = sorted_values[i];

    const [hours, minutes] = event.minute.split(":").map(val => parseInt(val));

    if (event.action.includes("shift")) {
        const id = parseInt(event.action.split(" ")[1].replace("#", ""));
        guard = id;
        ...
        guards[guard].minute = minutes;
    } else if(event.action === "wakes up") {
        guards[guard].asleep += (minutes - guards[guard].minute);
        const start = guards[guard].minute;
        const stop = minutes;
        for (let i = start; i < stop; i++) {
            if (i in guards[guard].sleeping) {
                guards[guard].sleeping[i]++;
            } else {
                guards[guard].sleeping[i] = 1;
            }
        }
    } else {
        console.log("WHOOPS");
...
const guards_array = Object.values(guards);

let sleeping_guard=0;
let minute=0;
let total=0;

for (let i = 0; i< guards_array.length; i++) {
    let guard_minute;
    let amount = 0;
    for (key in guards_array[i].sleeping) {
        if (guards_array[i].sleeping[key] > amount) {
            guard_minute = key;
            amount = guards_array[i].sleeping[key];
        }
    }
    if (amount > total) {
        sleeping_guard = guards_array[i].id;
        total = amount;
        minute = guard_minute;
    }
}

console.log(sleeping_guard, minute, total);
// console.log(Object.values(guards).reduce((prev, current) => {
//     return prev.asleep > current.asleep ? prev: current;
// }))
