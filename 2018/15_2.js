        ...
        for (let y = 0; y < grid.length; y++) {
            for (let x = 0; x < grid[y].length; x++) {
                if (grid[y][x][1]) {
                    ...
                    if (stop) {
                        // console.log(it, stop, sumHitpoints(grid)*it);
                        if (stop["E"] === 0 || elves !== stop["E"]) {
                            return false;
                        }
                        console.log("stop", it);
...
async function createBattlefield(ap=3) {
    let elves = 0;
    ...
    const input = (await getInput("./input_15.txt")).lines.map((line, y) => {
        return line.split("").map((el, x) => {
            ...
            if (el === "#" || el === ".") {
                ...
                cell[1] = new Creature(x, y, el, ((el === "E") ? ap : 3));
                if (el === "E") elves++;
            }
            return cell;
        });
    ...
    const result = simulate(input, elves);
...
async function main() {
    ...
    while(!won) {
        ap++;
        console.log(`attacking with ${ap} ap...`);
        won = await createBattlefield(ap);
    }
