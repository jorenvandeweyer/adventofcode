let x =`kvvfl kvvfl olud wjqsqa olud frc
...
function sameLetters(word, otherWord){
    for(let i = 0; i < word.length; i++){
        if(!otherWord.includes(word[i])){
            return false;
        } else {
            if(!(countInArray(otherWord, word[i]) == countInArray(word, word[i]))){
                return false;
            }
        }
    }
