...
input = input.split("\n");
input = input.map( x => parseString(x));
...
for(let i = 0; i < 500000; i++){
    for(let j = 0; j < input.length; j++){
        //console.log(Math.abs(input[j].p.x) + Math.abs(input[j].p.y) + Math.abs(input[j].p.z))
        input[j].v.x += input[j].a.x;
        input[j].v.y += input[j].a.y;
        input[j].v.z += input[j].a.z;

        input[j].p.x += input[j].v.x;
        input[j].p.y += input[j].v.y;
        input[j].p.z += input[j].v.z;

        if(i > 30000){
            if(Math.abs(input[j].p.x) + Math.abs(input[j].p.y) + Math.abs(input[j].p.z) <= distance){
                closest = j;
                distance = Math.abs(input[j].p.x) + Math.abs(input[j].p.y) + Math.abs(input[j].p.z);
...
function parseString(line){
    ...
    for(let i = 0; i < line.length; i++){
        let index = line[i][0];
        let item = line[i].split("<")[1].split(">")[0].split(",");
        point[index] = {
            x: parseInt(item[0]),
            y: parseInt(item[1]),
            z: parseInt(item[2])
