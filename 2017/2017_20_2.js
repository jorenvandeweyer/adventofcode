...
for(let i = 0; i < 50; i++){
    ...
    let keys = [];
    for(key in obj){
        obj[key].v.x += obj[key].a.x;
        obj[key].v.y += obj[key].a.y;
        obj[key].v.z += obj[key].a.z;

        obj[key].p.x += obj[key].v.x;
        obj[key].p.y += obj[key].v.y;
        obj[key].p.z += obj[key].v.z;

        let coords = obj[key].p.x + "|" + obj[key].p.y + "|" + obj[key].p.z;
        if(positions.includes(coords)){
            let index = positions.indexOf(coords);
